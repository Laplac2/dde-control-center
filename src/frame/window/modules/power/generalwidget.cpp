...
void GeneralWidget::initUi()
{
    ...
    powerPlansLabel->setContentsMargins(10, 0, 0, 0);
    ...
    /**FIXME
     * 为了使设置详情里的文字左边与上面标题对齐，这个边距应该是dtk改，等dtk改好了以后，这里的边距就可以删除或设置为0了
     */
    for (int i = 0; i < energySavingGrp->itemCount(); ++i) {
        energySavingGrp->layout()->itemAt(i)->widget()->setContentsMargins(5, 0, 0, 0);
    }
    energySavingLabel->setContentsMargins(10, 0, 0, 0);
     ...
     * 为了使设置详情里的文字左边与上面标题对齐，这个边距应该是dtk改，等dtk改好了以后，这里的边距就可以删除或设置为0了
     */
    for (int i = 0; i < wakeupSettingsGrp->itemCount(); ++i) {
        wakeupSettingsGrp->layout()->itemAt(i)->widget()->setContentsMargins(5, 0, 0, 0);
    }
    ...
    m_powerShowTimeToFull = new SwitchWidget(tr("Display capacity and remaining charging time"));
    m_ShowTimeToFullTips = new PowerDisplayWidget(tr("Maximum capacity"));
     ...
     * 为了使设置详情里的文字左边与上面标题对齐，这个边距应该是dtk改，等dtk改好了以后，这里的边距就可以删除或设置为0了
     */
    for (int i = 0; i < batterySettingsGrp->itemCount(); ++i) {
        batterySettingsGrp->layout()->itemAt(i)->widget()->setContentsMargins(5, 0, 0, 0);
    }
    ...
    batteryLabel->setContentsMargins(10, 0, 0, 0);
